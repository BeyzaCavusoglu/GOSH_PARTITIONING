...
int mm_read_banner(FILE *f, MM_typecode *matcode)
{
    ...
    if (sscanf(line, "%s %s %s %s", mtx, crd, data_type, storage_scheme) != 4)
    {
        // Assume no banner and reset the file pointer
        rewind(f);
        strcpy(mtx, MM_MTX_STR);
        strcpy(crd, MM_SPARSE_STR);
        strcpy(data_type, MM_REAL_STR);
        strcpy(storage_scheme, MM_GENERAL_STR);
    }
    ...
    // Set the typecode based on the fields
    mm_set_matrix(matcode);
